...
除此之外还有一个全局`module path`，是当前node执行文件的相对目录（`../../lib/node`）。如果在环境变量中设置了`HOME`目录和`NODE_PATH`目录的话，整个路径还包含`NODE_PATH`和`HOME`目录下的`.node_libraries`与`.node_modules`。其最终值大致如下：
```
